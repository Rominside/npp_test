...
def handle_event(id: str, details: dict):    
    ...
    try:
            details.update({"authorized": check_operation(id, details)})
            check = "False"
            if details["authorized"]:
                check = "True"
            print(f"result of checking {check}")
            # remove update url details as it will not be further used
            # del details['url']
            if details["authorized"]:
                if details["operation"] != "pulled_out_key" and details["operation"] != "insert_key":
                    if details["operation"] == "alarm" or details["operation"] == "update_coef_complit" or\
                        details["operation"] == "log" or details["operation"] == "successful_check":
                            # details["target"] = "file_system"
        ...
        while True:
            ...
            if msg is None:
                ...
                try:
                    ...
                    print(f"Malformed event received from topic {topic}: {msg.value()}. {e}")
